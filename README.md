...
- Configure the API target by copying `packages/web/.env.example` to `packages/web/.env` or setting `VITE_RUST_SERVER_QUERY_API_URL` when building the SPA. For example:

```
VITE_RUST_SERVER_QUERY_API_URL="https://www.deltaserve.nl/api" yarn build:web
```
